...
class Block(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    # What is residual? That made above one wrong?: ResNet
...
class PatchEmbed(nn.Module):
  def __init__(
    ...
    self.patch_size = to_2tuple(patch_size) #we don't need it: pytorch handles it automatically
    self.image_size, self.grid_size, self.num_patches = self._init_img_size(image_size)
    self.flatten = flatten

    # Hugging face is genius - no, they also stole (not exactly) it from google
    # number of convolution filters = embed_dim
    # or in linear layer form -> patch shape = (c, p, p) -> flattened to c * p * p
    # We have a linear layer with #neurons = embed_dim and each with #weights = c * p * p
    # Repeats for all patches
